...
#if EBOS_USE_ALUGRID
NEW_TYPE_TAG(EclBaseProblemAlu, INHERITS_FROM(EclAluGridVanguard, EclOutputBlackOil, VtkEclTracer));
#else
NEW_TYPE_TAG(EclBaseProblemAlu, INHERITS_FROM(EclCpGridVanguard, EclOutputBlackOil, VtkEclTracer));
...
SET_TYPE_PROP(EclBaseProblemAlu, Problem, Opm::EclProblemAlu<TypeTag>);

// Select the element centered finite volume method as spatial discretization
SET_TAG_PROP(EclBaseProblemAlu, SpatialDiscretizationSplice, EcfvDiscretization);

//! for ebos, use automatic differentiation to linearize the system of PDEs
SET_TAG_PROP(EclBaseProblemAlu, LocalLinearizerSplice, AutoDiffLocalLinearizer);

// Set the material law for fluid fluxes
SET_PROP(EclBaseProblemAlu, MaterialLaw)
{
private:
...
SET_PROP(EclBaseProblemAlu, SolidEnergyLaw)
{
private:
    typedef typename GET_PROP_TYPE(TypeTag, Scalar) Scalar;
    typedef typename GET_PROP_TYPE(TypeTag, FluidSystem) FluidSystem;
...
SET_PROP(EclBaseProblemAlu, ThermalConductionLaw)
{
private:
    typedef typename GET_PROP_TYPE(TypeTag, Scalar) Scalar;
    typedef typename GET_PROP_TYPE(TypeTag, FluidSystem) FluidSystem;
...
SET_PROP(EclBaseProblemAlu, Stencil)
{
private:
    typedef typename GET_PROP_TYPE(TypeTag, Scalar) Scalar;
...
SET_TYPE_PROP(EclBaseProblemAlu, EclAquiferModel, Opm::EclBaseAquiferModel<TypeTag>);

// use the built-in proof of concept well model by default
SET_TYPE_PROP(EclBaseProblemAlu, EclWellModel, EclWellManager<TypeTag>);

// Enable aquifers by default in experimental mode
SET_BOOL_PROP(EclBaseProblemAlu, EclEnableAquifers, true);

// Enable gravity
SET_BOOL_PROP(EclBaseProblemAlu, EnableGravity, true);

// only write the solutions for the report steps to disk
SET_BOOL_PROP(EclBaseProblemAlu, EnableWriteAllSolutions, false);

// disable API tracking
SET_BOOL_PROP(EclBaseProblemAlu, EnableApiTracking, false);
...
SET_SCALAR_PROP(EclBaseProblemAlu, EndTime, 1e100);
...
SET_SCALAR_PROP(EclBaseProblemAlu, InitialTimeStepSize, 3600*24);

// the default for the allowed volumetric error for oil per second
SET_SCALAR_PROP(EclBaseProblemAlu, NewtonTolerance, 1e-2);

// the tolerated amount of "incorrect" amount of oil per time step for the complete
// reservoir. this is scaled by the pore volume of the reservoir, i.e., larger reservoirs
// will tolerate larger residuals.
SET_SCALAR_PROP(EclBaseProblemAlu, EclNewtonSumTolerance, 1e-4);
...
SET_SCALAR_PROP(EclBaseProblemAlu, EclNewtonSumToleranceExponent, 1.0/3.0);

// set number of Newton iterations where the volumetric residual is considered for
// convergence
SET_INT_PROP(EclBaseProblemAlu, EclNewtonStrictIterations, 8);

// set fraction of the pore volume where the volumetric residual may be violated during
// strict Newton iterations
SET_SCALAR_PROP(EclBaseProblemAlu, EclNewtonRelaxedVolumeFraction, 0.03);

// the maximum volumetric error of a cell in the relaxed region
SET_SCALAR_PROP(EclBaseProblemAlu, EclNewtonRelaxedTolerance, 1e9);

// Ignore the maximum error mass for early termination of the newton method.
SET_SCALAR_PROP(EclBaseProblemAlu, NewtonMaxError, 10e9);
...
SET_INT_PROP(EclBaseProblemAlu, NewtonMaxIterations, 14);
...
SET_INT_PROP(EclBaseProblemAlu, NewtonTargetIterations, 6);

// Disable the VTK output by default for this problem ...
SET_BOOL_PROP(EclBaseProblemAlu, EnableVtkOutput, false);

// ... but enable the ECL output by default
SET_BOOL_PROP(EclBaseProblemAlu, EnableEclOutput, true);

// If available, write the ECL output in a non-blocking manner
SET_BOOL_PROP(EclBaseProblemAlu, EnableAsyncEclOutput, true);

// By default, use single precision for the ECL formated results
SET_BOOL_PROP(EclBaseProblemAlu, EclOutputDoublePrecision, false);

// The default location for the ECL output files
SET_STRING_PROP(EclBaseProblemAlu, OutputDir, ".");

// the cache for intensive quantities can be used for ECL problems and also yields a
// decent speedup...
SET_BOOL_PROP(EclBaseProblemAlu, EnableIntensiveQuantityCache, true);

// the cache for the storage term can also be used and also yields a decent speedup
SET_BOOL_PROP(EclBaseProblemAlu, EnableStorageCache, true);

// Use the "velocity module" which uses the Eclipse "NEWTRAN" transmissibilities
SET_TYPE_PROP(EclBaseProblemAlu, FluxModule, Opm::EclTransFluxModule<TypeTag>);

// Use the dummy gradient calculator in order not to do unnecessary work.
SET_TYPE_PROP(EclBaseProblemAlu, GradientCalculator, Opm::EclDummyGradientCalculator<TypeTag>);
...
SET_TYPE_PROP(EclBaseProblemAlu, NewtonMethod, Opm::EclNewtonMethod<TypeTag>);

// The frequency of writing restart (*.ers) files. This is the number of time steps
// between writing restart files
SET_INT_PROP(EclBaseProblemAlu, RestartWritingInterval, 0xffffff); // disable
...
SET_BOOL_PROP(EclBaseProblemAlu,
              EclEnableDriftCompensation,
              GET_PROP_VALUE(TypeTag, EnableExperiments));

// By default, we enable the debugging checks if we're compiled in debug mode
SET_BOOL_PROP(EclBaseProblemAlu, EnableDebuggingChecks, true);

// store temperature (but do not conserve energy, as long as EnableEnergy is false)
SET_BOOL_PROP(EclBaseProblemAlu, EnableTemperature, true);

// disable all extensions supported by black oil model. this should not really be
// necessary but it makes things a bit more explicit
SET_BOOL_PROP(EclBaseProblemAlu, EnablePolymer, false);
SET_BOOL_PROP(EclBaseProblemAlu, EnableSolvent, false);
SET_BOOL_PROP(EclBaseProblemAlu, EnableEnergy, false);
SET_BOOL_PROP(EclBaseProblemAlu, EnableFoam, false);

// disable thermal flux boundaries by default
SET_BOOL_PROP(EclBaseProblemAlu, EnableThermalFluxBoundaries, false);

SET_BOOL_PROP(EclBaseProblemAlu, EnableTracerModel, false);

// By default, simulators derived from the EclBaseProblemAlu are production simulators,
// i.e., experimental features must be explicitly enabled at compile time
SET_BOOL_PROP(EclBaseProblemAlu, EnableExperiments, false);

// set defaults for the time stepping parameters
SET_SCALAR_PROP(EclBaseProblemAlu, EclMaxTimeStepSizeAfterWellEvent, 3600*24*365.25);
SET_SCALAR_PROP(EclBaseProblemAlu, EclRestartShrinkFactor, 3);
SET_BOOL_PROP(EclBaseProblemAlu, EclEnableTuning, false);

SET_STRING_PROP(EclBaseProblemAlu, OutputMode, "all");
 ...
 */
template <class TypeTag>
class EclProblemAlu : public GET_PROP_TYPE(TypeTag, BaseProblem)
{
...
public:
     ...
     */
    EclProblemAlu(Simulator& simulator)
     ...
     *
     * The EclProblemAlu does not write any restart files using the ad-hoc format, only ones
     ...
     *
     * For the EclProblemAlu, this method is identical to referencePorosity(). The intensive
...
template <class TypeTag>
std::string EclProblemAlu<TypeTag>::briefDescription_;
