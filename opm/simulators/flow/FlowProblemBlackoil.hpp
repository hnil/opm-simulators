...
public:
    ...
    void finishInit()
    {
        ...
        // Seed the DRSDT/DRVDT history from the initial composition. Left at zero, the
        // limiter caps Rs at zero on the first step and boils all the dissolved gas out
        // of an undersaturated reservoir. The restart path seeds it from the restart
        // solution instead.
        if (!initconfig.restartRequested()) {
