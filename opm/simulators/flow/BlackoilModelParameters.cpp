...
BlackoilModelParameters<Scalar>::BlackoilModelParameters()
{
    ...
    network_owns_individual_controls_ = Parameters::Get<Parameters::NetworkOwnsIndividualControls>();
...
void BlackoilModelParameters<Scalar>::registerParameters()
{
    ...
    Parameters::Register<Parameters::NetworkOwnsGroupControl>
        ("Let the production network solve decide group control on its own: every well the "
         "deck makes available for group control under a group with a target is placed by the "
         "solve, whatever control it is on now; the legacy network evaluation and the group "
    ...
    Parameters::Register<Parameters::NetworkOwnsIndividualControls>
        ("With --network-owns-group-control, also skip the individual constraint checks (thp, bhp, "
         "rate) for the wells the network controls -- in the outer control update and inside the "
         "well solves -- so only the network solve switches them");
