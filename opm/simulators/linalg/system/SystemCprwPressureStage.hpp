        ...
        *coarseMatrix_ = 0.0;
        ...
        for (std::size_t j = 0; j < layout.numWells(); ++j) {
            ...
            for (std::size_t wb = layout.firstBlock(j); wb < layout.endBlock(j); ++wb) {
                ...
                // straight into the diagonal instead of paying for a
                // wellOfBlock() lookup to re-derive the well index we already
                // have as the loop variable j. This picks up the full
                // segment-to-segment coupling rather than just the top
                // segment's column.
                for (auto col = D[wb].begin(), colEnd = D[wb].end(); col != colEnd; ++col) {
                    assert(col.index() >= layout.firstBlock(j) && col.index() < layout.endBlock(j));
                    Scalar el = 0.0;
                    for (std::size_t i = 0; i < lw.size(); ++i) {
                        el += lw[i] * (*col)[i][q];
                    }
