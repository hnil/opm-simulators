                         ...
                         const WellState<Scalar, IndexTraits>& well_state) const
{
    ...
    if (!well.isPressureControlled(well_state)) {
        ...
        if (contract_d_diagonal) {
            ...
            // segment pressure, while the row sum estimates the physical bhp
            // derivative; without the same factor the coarse well column and
            // diagonal disagree by the scale. The contracted diagonal inherits
            // it from D itself.
