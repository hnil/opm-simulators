...
namespace Opm
{
    ...
    , ipr_b_(number_of_phases_)
    {
        ...
        duneCA_.setBuildMode( OffDiagMatWell::row_wise );
	...
	invDuneD_[0][0].resize(numWellEq_, numWellEq_);
	    ...
	    duneD_[0][0].resize(numWellEq_, numWellEq_);
        }
	...
	for (int perf = 0; perf < number_of_perforations_; ++perf) {
	    ...
	    for (int perf = 0; perf < number_of_perforations_; ++perf) {
		const int cell_idx = well_cells_[perf];
		duneCA_[0][cell_idx].resize(numWellEq_, numWellAdjoint);
	    }
        }
	    ...
	    duneDA_[0][0].resize(numWellEq_, numWellAdjoint);
        }
        ...
        if(numWellAdjoint>0){
	    // adjoint vairables of well
            adjWell_.resize(1);
	    adjWell_[0].resize(numWellEq_);
	    // help values for operator solve
	    Ctx_.resize( duneC_.M());
	    for (unsigned i = 0; i < duneC_.M(); ++i) {
		Ctx_[i].resize(numWellEq_);
	    }
            invDtadj_.resize( invDuneD_.N());
	    for (unsigned i = 0; i < invDuneD_.N(); ++i) {
		invDtadj_[i].resize(numWellEq_);
	    }
	    // objective derivatives
            objder_.resize(1);
	    objder_[0].resize(numWellAdjoint);
	    // of size reservoir not dynamic
	...
	assert(well_state.currentControls()[index_of_well_] == well_controls_get_current(this->well_controls_));
		...
		if(numWellAdjoint>0){// NB we should probably also have a runtime switch here: should be extended to not only controIndex for later use
        ...
        try {
	    // NB this statement have no effect since one of the methods used in flow trow anything
	    // it either fake an inverse or give nan and inf
	    // TODO make proper error handeling, since none invertable matrix indicate an logical error
    ...
    assembleControlEq(Opm::DeferredLogger& deferred_logger)
    {
        ...
        if (numWellAdjoint > 0) {
	    // Currently only one adjoint variable for each well
