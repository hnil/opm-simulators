...
{
    // the length was packed as a std::size_t, so it must be unpacked into a
    // std::size_t as well (unpacking into an int makes MPI_Unpack write 8
    // bytes into a 4 byte variable, corrupting the stack)
    std::size_t length = 0;
    int int_position = 0;
    ...
    MPI_Unpack(buffer.data()+position, mpi_buffer_size(buffer.size(), position),
               &int_position, cStr.data(), static_cast<int>(length),
               MPI_CHAR, comm);
    position += int_position;
    data.clear();
