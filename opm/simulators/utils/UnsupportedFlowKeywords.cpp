...
const KeywordValidation::UnsupportedKeywords& unsupportedKeywords()
{
    static const KeywordValidation::UnsupportedKeywords unsupported_keywords = {
        ...
        {"COARSEN", {true, "Only refinement (CARFIN) is implemented, not coarsening."}},
        {"COLLAPSE", {true, std::nullopt}},
        {"COLUMNS", {true, std::nullopt}},
        {"CBMOPTS", {true, std::nullopt}},
        {"COMPDATX", {true, std::nullopt}},
        {"COMPDATM", {true, "A multisegment well cannot be completed in a refined block. Complete the well with COMPDATL, or place the box so it does not cover the well."}},
        ...
        {"COMPSEGL", {true, "A multisegment well cannot be completed in a refined block: the segment-to-cell mapping in LGR-local indices is not implemented. Complete the well with COMPDATL, or place the box so it does not cover the well."}},
        ...
        {"IMPES", {false, std::string{"Flow only supports the fully implicit method and will use it instead"}}},
        ...
        {"LGRCOPY", {false, "The refinement is taken from the CARFIN records; refined cells inherit their parent cell's properties."}},
        {"LGRFREE", {true, "Refinement is static for the whole run: a block cannot be released during it."}},
        {"LGRLOCK", {false, "Refinement is static for the whole run."}},
        ...
        {"RADFIN", {true, "Only Cartesian refinement (CARFIN) is implemented, not radial."}},
        {"RADFIN4", {true, "Only Cartesian refinement (CARFIN) is implemented, not radial."}},
        ...
        {"REFINE", {true, "Refinement is specified with CARFIN."}},
        {"RADFIN4", {true, "Only Cartesian refinement (CARFIN) is implemented, not radial."}},
